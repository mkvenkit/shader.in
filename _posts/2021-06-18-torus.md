...
You can think of tangent space as a local coordinate system attached to each point *P(x, y, z)* on the torus. At this point, there are three vectors that are orthogonal to each other - *T<sub>u</sub>* - the tangent along the *u* direction. *T<sub>v</sub>* the tangent along the *v* dirtection, and the normal *N*. The coordinate system is show below.
...
So we do need the tanget, and it is computed by taking the partial derivative of the surface *P* as follows:
...
Here's the code, from the *Torus::_createTorus()* method which computes the vertices, normals, and tangents. Note that we use *std::vector<float>* to store the geometry, which is very convenient, and has the same perforance as using a C++ array.
...
*Ka* and *Ia* are the material color and intensity of *ambient* light - you can think of this as a direction-less 

Now let's look at specific shading techniques to compute the final color.

# Normal Matrix 

Vertices are transformed using the *model* and *view* transformations before applying the *projection* transform. But perhaps surprisingly, Normals do not quite transform the same way. They transform as:
...
Although GLSL has *inverse* and *transpose* functions, We will compute *M<sub>n</sub>* in the C++ code and pass it to the shader using a *uniform*, since we don't want the redundant computation to run on every single vertex in our geometry.
