...
Since the color is only computed at the vertices, Gouraud shading misses out on some things. For example, if there's a shiny spot in the center of a triangle, the interpolation that happens in the fragment shader will miss it. That's where Phong shading comes in.
