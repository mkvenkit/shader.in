...
To get some intution on Eq. 2, think about the direction of the normals on a circular ring on the torus. They will be the same even if the ring is translated to the origin. Also, remember that normal at a point $$P=(x, y, z)$$ is $$N=(x, y, z)$$. So if you set $$R=0$$ and $$r=1$$ in Eq. 1, you end up with Eq. 2.

## Tangent Space



For reasons which will be apparent soon, we also need the **tangent
